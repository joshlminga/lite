...
class ListingLogos extends CI_Controller {
	...
	*/
	...
	private $SubFolder = '/logos'; //Set Default Sub Folder For html files and Front End Use Start with /
	...
	private $Route = 'listing_logos'; //If you have different route Name to Module name State it here |This wont be pluralized

	private $New = 'listing_logos/new'; //New 
	private $Save = 'listing_logos/save'; //Add New 
	private $Edit = 'listing_logos/update'; //Update 

	private $ModuleName = 'logos';
    ...
    */
	public function index($notifyMessage=null)
	{
		...
		//Table Select & Clause
	   	$columns = array('id as id,data as company_name,flg as status');
	   	$where = array('title' => 'Logos');
	...
	public function valid($type)
	{
		...
		if ($type == 'save') {
			...
			if ($this->validation($formData) == TRUE) {
				...
				$uploaded = $this->CoreCrud->uploadFile($_FILES['logo'],'jpg|jpeg|png',$upoadDirectory,true);
				if (!is_null($uploaded)) {
					$formData['logo'] = json_encode($uploaded, true);
				}else{
					$formData['logo'] = json_encode(array());
				}
		...
		elseif ($type == 'update') {
			...
			if ($this->validation($updateData,false,array($column_password)) == TRUE) {
				...
				if (!empty($_FILES['logo']['name'][0])) {
					...
					$uploaded = $this->CoreCrud->uploadFile($_FILES['logo'],'jpg|jpeg|png',$upoadDirectory,true);
					if (!is_null($uploaded)) {
						$updateData['logo'] = json_encode($uploaded, true);
					}else{
						$updateData['logo'] = json_encode(array());
					}
				}else{
					//UnSet Data
					...
					//Photos
					if (array_key_exists('logo', $current_details)) {
						$photos = $current_details['logo'];
					}else{
						$photos = array();
					}
					$updateData['logo'] = json_encode($photos);
				}
...
/* End of file ListingLogos.php */
/* Location: ./application/controllers/ListingLogos.php */