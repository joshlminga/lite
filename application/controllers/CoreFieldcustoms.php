...
class CoreFieldcustoms extends CI_Controller {
	...
	*/
	...
	private $Module = 'fieldcustom'; //Module
	private $Folder = 'fieldcustom'; //Set Default Folder For html files and Front End Use
	...
	private $Unique = 'group'; // Unique & Required Values During Form Validation
	...
	private $New = 'fieldcustoms/new'; //New 
	private $Save = 'fieldcustoms/save'; //Add New 
	private $Edit = 'fieldcustoms/update'; //Update 
    ...
    */
	public function index($notifyMessage=null)
	{
		...
		$customFieldTable = $this->plural->pluralize('customfield');
		...
		//Table Select & Clause
	   	...
	   	$where = array('flg' =>1);
		$data['dataList'] = $this->CoreCrud->selectCRUD($customFieldTable,$where,$columns);
	...
	* 
    */
	public function open($pageID,$message=null,$layout='main',$type=null)
	{
		...
		if (!is_null($type)) {
			...
			//Column Type
			$customFieldTable = $this->plural->pluralize('customfield'); // Custom Fields Table
			//Table Select & Clause
	   		$columns = array('type as field_type,parent as parent_field,child as children');
		   	$where = array('type' => $type);
			$data['resultList'] = $this->CoreCrud->selectCRUD($customFieldTable,$where,$columns,'like');
		}
	...
	*/
	public function edit($pageID,$inputTYPE='id',$inputID=null,$message=null,$layout='main')
	{
		...
		if (!is_null($inputTYPE) || !is_null($inputID)) {
			...
			//Type
			$type = $this->db->select('fieldcustom_type')->where('fieldcustom_id',$inputID)
			->get($this->plural->pluralize($module))->row()->fieldcustom_type;
			...
			//Table Select & Clause
			$where = array('id' =>$inputID);
	   		$columns = array('id as id,group as group_list,type as type_list,parent as parent_list,child as children_list');
	...
	/*
	*
	*  This function is to be called when you want to pass the Manage form
    * In here we can call the load function and pass data to passed as an array inorder to manupulate it inside passed function
    * 	* Set your Page name/ID here N:B Page ID can be a number if you wish to access other values linked to the page opened E.g Meta Data
    * 	* You can also set Page ID as actual pageName found in your view N:B do not put .php E.g home.php it should just be 'home'
    * 	* Set Page template 
	...
	* 
	*/
	public function manage($pageID,$inputTYPE='id',$inputID=null,$message=null,$layout='main')
	{
		//Pluralize Module
		$module = $this->plural->pluralize($this->Module);
		...
		if (!is_null($inputTYPE) || !is_null($inputID)) {
			//Column Type
			$customFieldTable = $this->plural->pluralize('customfield'); // Custom Fields Table
			$type = $this->db->select('customfield_type')->where('customfield_id',$inputID)->get($customFieldTable)->row()->customfield_type;

			//Table Select & Clause
		   	$columns = array('id as id,group as item_group,parent as parent_field,flg as status');
		   	$where = array('type' => $type);
			$data['dataList'] = $this->CoreCrud->selectCRUD($module,$where,$columns,'like');
			$data['fieldType'] = $type;

			//Form Submit URLs
			$data['form_new'] = $this->New."/$type";
			...
			//Error Edit | Load the Manage Page
			$this->open($message='System could not find the detail ID');
		}
	}
	...
	*/
	public function valid($type)
	{
		...
		if ($type == 'save') {
			...
			$typeField = $this->CoreLoad->input('fieldcustom_type'); //Input Type

			//Form Validation
			if ($this->validation($formData) == TRUE) {
				//Prepaire Data To Store
				foreach ($formData as $key => $value) {
					if ($key !== 'fieldcustom_type' &&  $key !== 'fieldcustom_parent' && $key !== 'fieldcustom_group') {
						$children[$key] = $value;
				...
				$child_json_list = json_encode($children); //Set Childreans
				$childArray = json_decode($child_json_list, True);
				$childArray = $this->CoreField->allign_children($childArray);
				$children = json_encode($childArray); //Set Childreans

				$formData[$column_child] = $children; //Childrens

				if ($this->create($formData)) {
					$this->session->set_flashdata('notification','success'); //Notification Type
					$message = 'Successful'; //Notification Message				
					redirect("$coreModule/open/add/$message/main/$typeField","refresh");//Redirect to Page
				}else{
					$this->session->set_flashdata('notification','error'); //Notification Type
					$this->open('add',null,'main',$typeField);//Open Page
				}
			}else{
		...
		elseif ($type == 'update') {
			...
			//Prepaire Data To Update
			foreach ($updateData as $key => $value) {
				if ($key !== 'fieldcustom_type' &&  $key !== 'fieldcustom_parent' && $key !== 'fieldcustom_group' && $key !== 'id') {
					$children[$key] = $value;
			...
			$updateData[$column_child] = $children; //Childrens
...
/* End of file CoreFieldcustoms.php */
/* Location: ./application/controllers/CoreFieldcustoms.php */