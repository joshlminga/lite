...
class _baseController extends CI_Controller {
	...
	*/
	...
	private $Core = '/* Core here place the word Core | Or Leave Empty For Extensions*/'; //Lite Main Core
	private $Module = '/* Module name in singular (the table name) */'; //Module
	private $Folder = '/* HTML Source Folder Name */'; //Set Default Folder For html files and Front End Use
	private $SubFolder = '/* HTML Source Sub Folder Name - start with / */'; //Set Default Sub Folder For html files and Front End Use Start with /
	...
	private $New = '/* route for add new item form */'; //New 
    ...
    */
	public function index($notifyMessage=null)
	{
		...
		//Table Select & Clause
	   	$columns = array('id as id,flg as status');
	...
	*/
	public function valid($type)
	{
		...
		if ($type == 'save') {
			...
			/* You can add the upload session above rules, to handle multiple Uploads - check below */
			$this->form_validation->set_rules($this->session->file_name, 'Upload Label', 'trim|callback_validation'); //Demo Custom Validation

			//Set Upload File Values 2
			$file_upload_session = array("file_name" => "upload_file_name2", "file_required" => true);
			$this->session->set_userdata($file_upload_session);
			$this->form_validation->set_rules($this->session->file_name, 'Upload Label2', 'trim|callback_validation'); //Demo Multiple Custom Validation

			//Set Upload File Values 3
			$file_upload_session = array("file_name" => "upload_file_name3", "file_required" => true);
			$this->session->set_userdata($file_upload_session);
			$this->form_validation->set_rules($this->session->file_name, 'Upload Label3', 'trim|callback_validation'); //Demo Multiple Custom Validation
