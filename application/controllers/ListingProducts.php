...
class ListingProducts extends CI_Controller {
	...
	*/
	...
	private $SubFolder = '/products'; //Set Default Sub Folder For html files and Front End Use Start with /
	...
	private $Route = 'listing_products'; //If you have different route Name to Module name State it here |This wont be pluralized

	private $New = 'listing_products/new'; //New 
	private $Save = 'listing_products/save'; //Add New 
	private $Edit = 'listing_products/update'; //Update 
	...
	*/
	public function passed($values=null)
	{
		...
		$where = array('listinglist_flg' =>1,'listinglist_title' =>'Companies');
		$data['companies'] = $this->db->select('listinglist_id,listinglist_data')->where($where)->get($this->Module)->result();
    ...
    */
	public function index($notifyMessage=null)
	{
		...
		//Table Select & Clause
	   	$columns = array('id as id,data as product_name,data as product_cost,flg as status');
	   	$where = array('title' => 'Products');
	...
	public function valid($type)
	{
		...
		if ($type == 'save') {
			...
			if ($this->validation($formData) == TRUE) {
				...
				$upoadDirectory = "../assets/admin/images/upload/$this->Folder$this->SubFolder";
				$uploaded = $this->CoreCrud->uploadFile($_FILES['photos'],'jpg|jpeg|png',$upoadDirectory,true);
				if (!is_null($uploaded)) {
					$formData['photos'] = json_encode($uploaded, true);
				}else{
					$formData['photos'] = json_encode(array());
				}
		...
		elseif ($type == 'update') {
			...
			if ($this->validation($updateData,false,array($column_password)) == TRUE) {
				...
				if (!empty($_FILES['photos']['name'][0])) {
					...
					$uploaded = $this->CoreCrud->uploadFile($_FILES['photos'],'jpg|jpeg|png',$upoadDirectory,true);
					if (!is_null($uploaded)) {
						$updateData['photos'] = json_encode($uploaded, true);
					}else{
						$updateData['photos'] = json_encode(array());
					}
				}else{
					//UnSet Data
					//$updateData = $this->CoreLoad->unsetData($updateData,array('photos'));
					$columnData = strtolower($this->CoreForm->get_column_name($this->Module,'data'));
					$current_details = json_decode($this->db->select($columnData)->where(array($column_id =>$value_id))->get($tableName)
					->row()->$columnData, true);

					//Photos
					if (array_key_exists('photos', $current_details)) {
						$photos = $current_details['photos'];
					}else{
						$photos = array();
					}
					$updateData['photos'] = json_encode($photos);
				}
...
/* End of file ListingProducts.php */
/* Location: ./application/controllers/ListingProducts.php */