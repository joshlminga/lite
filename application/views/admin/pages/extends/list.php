            							...
            							<?php for ($i = 0; $i < count($dataList); $i++) : ?>
            								<tr>
            									<?php for ($k = 0; $k < count($ths); $k++) :  $key = $ths[$k]; ?>
            										...
            										<?php elseif ($key == 'type') : ?>
            											<?php
														// Explode and get the first element of the array
														$type = explode('_', $dataList[$i]->$key);
														$type = $type[0];
														?>
            											<td> <?= ucwords($type); ?> </td>
            										...
            										<?php elseif ($key == 'name') : ?>
														...
														$settingInfo = json_decode($sett_value);
														// check if property name exist in settingInfo object
														if (property_exists($settingInfo, 'name')) {
															$name = $settingInfo->name;
														} elseif (property_exists($settingInfo, 'menu_path')) {
															$name = $settingInfo->menu_path;
															// replace extend/controls/
															$name = str_replace('extend/controls/', '', $name);
															// replace / with space
            										...
            										<?php elseif ($key == 'path') : ?>
														...
														$sett_value = $this->CoreCrud->selectSingleValue('settings', 'value', ['id' => $dataList[$i]->$key]);
														$settingInfo = json_decode($sett_value);
														// check if property menu_path exist in settingInfo object
														if (property_exists($settingInfo, 'menu_path')) {
															$ext_dir = $this->CoreCrud->selectSingleValue('settings', 'value', ['title' => 'ext_dir']);
															$title = $this->CoreCrud->selectSingleValue('settings', 'title', ['id' => $dataList[$i]->$key]);

															// title check if title = extension_menu
															if ($title == 'extension_menu') {
																$path = $ext_dir . 'extensions/' . $settingInfo->menu_path;
															} elseif ($title == 'field_menu') {
																$path = $ext_dir . 'customfields/' . $settingInfo->menu_path;
															} else {
																$path = $settingInfo->menu_path;
															}
															// Explode and remove the last element of the array then implode it
